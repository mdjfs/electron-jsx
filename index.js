const {stat, writeFile, watchFile, readdir, mkdir, existsSync, copyFile} = require("fs");
...
function mainCheckProcess({reactDir, main = "index.jsx"}){
    if(reactDir){
        check({dirs: [reactDir]}, err => {
            const mainPath = path.join(reactDir, main);
            if(!err) check({files: [mainPath]}, err => {
                if(!err){
                    renderDev(__dirname, reactDir, mainPath);
                }
            }) 
        });
    }
        ...
        readdir(folder, { withFileTypes: true }, (err, data) => {
            if(err) write(err, true)
                ...
                for(var file of files){
                    ...
                    else streamFile(file, file.replace(reactDir,distDir), true, false);
                }
            }
        })
    }
...
function streamFile(inputPath, outputPath, listen=true, replace=true){
    if(replace) copyFile(inputPath, outputPath, (err) => {if(err) write(err, true)});
    else copyFile(inputPath, outputPath, COPYFILE_EXCL,  (_) => {});
    if(listen) watchFile(inputPath, _ => { write(`Copying ${path.basename(inputPath)} ... :P`); streamFile(inputPath, outputPath,false)});
}

function transformJSX(inputPath, outputPath, listen=true, writeOptions={flag: 'wx'}){
    babel.transformFile(inputPath, options, (err, data) => { 
        if(err) write(err, true);
        else writeFile(outputPath, data.code, writeOptions, (_)=>{});
    });
    if(listen) watchFile(inputPath, _ => {  write(`Compiling ${path.basename(inputPath)} ... :D`); transformJSX(inputPath, outputPath, false, {flag: 'w'}); });
}
